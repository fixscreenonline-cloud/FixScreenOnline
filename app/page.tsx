"use client";

import { Link } from "@heroui/link";
import { useState } from "react";
import Image from "next/image";
import {
  MessageCircle,
  Search,
  FileText,
  CheckCircle,
  Wrench,
  FlaskConical,
  Package,
  Headphones,
  Droplet,
  Cpu,
  HardDrive,
  Database,
  ChevronDown,
} from "lucide-react";

// Google Ads / gtag conversion helper
function gtagEvent(eventName: string, params?: Record<string, string>) {
  if (typeof window !== "undefined" && (window as any).gtag) {
    (window as any).gtag("event", eventName, params);
  }
}

import { GlowingEffect } from "@/components/ui/glowing-effect";
import { InfiniteMovingCards } from "@/components/ui/infinite-moving-cards";
import type { BookingFormData } from "@/components/booking-modal";
import { PageSection } from "@/components/page-section";
import { ServiceQuoteBlock } from "@/components/service-quote-block";
import { useLanguage } from "@/lib/i18n/language-provider";

const testimonialImages = [
  "https://i.ibb.co/r4g9Hbv/oie-v-KM22z-HFO6-SM-1-2.png",
  "https://i.ibb.co/dP2jN76/oie-v-KM22z-HFO6-SM-1-1.png",
  "https://i.ibb.co/bW0MNGF/oie-v-KM22z-HFO6-SM-1-3.png",
  "https://i.ibb.co/r4g9Hbv/oie-v-KM22z-HFO6-SM-1-2.png",
  "https://i.ibb.co/dP2jN76/oie-v-KM22z-HFO6-SM-1-1.png",
];

const processStepIcons = [
  MessageCircle,
  Search,
  FileText,
  CheckCircle,
  Wrench,
  FlaskConical,
  Package,
  Headphones,
] as const;

export default function Home() {
  const { dict } = useLanguage();
  const t = dict.home;
  const form = t.form.modal;
  const faqs = t.faq.items;
  const devices = t.form.devices;
  const testimonials = t.testimonials.items.map((item, index) => ({
    ...item,
    image: testimonialImages[index],
  }));
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  const submitBooking = async (data: BookingFormData): Promise<boolean> => {
    try {
      const response = await fetch("/api/send-enquiry", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(data),
      });

      if (response.ok) {
        gtagEvent("conversion", {
          send_to: process.env.NEXT_PUBLIC_GADS_CONVERSION_LABEL || "",
        });
        gtagEvent("form_submit", {
          event_category: "lead",
          event_label: "service_enquiry",
        });
        gtagEvent("generate_lead", {
          event_category: "engagement",
          event_label: `book_service_${data.serviceLocation}`,
        });
        return true;
      }
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error("Error submitting form:", error);
    }

    return false;
  };

  return (
    <div className="flex flex-col overflow-x-hidden">
      {/* Hero Section */}
      <section
        className="relative w-full overflow-x-hidden bg-gradient-to-b from-violet-50/50 via-white to-white py-6 sm:py-16 lg:flex lg:min-h-screen lg:items-center lg:overflow-hidden lg:py-24"
        id="hero"
      >
        {/* Blurred background elements for hero */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute top-20 left-20 w-96 h-96 bg-gradient-to-br from-violet-500/20 to-purple-500/20 rounded-full blur-3xl" />
          <div className="absolute bottom-20 right-20 w-96 h-96 bg-gradient-to-br from-blue-500/20 to-cyan-500/20 rounded-full blur-3xl" />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-full blur-3xl" />
        </div>

        {/* Floating service badges — desktop only */}
        <div className="hidden lg:contents">
        <div className="absolute top-[10%] sm:top-20 right-2 sm:right-10 bg-white/80 backdrop-blur-sm px-2 py-1.5 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl shadow-md sm:shadow-lg border border-violet-200/50 pointer-events-none block">
          <div className="flex items-center gap-1 sm:gap-2">
            <div className="w-6 h-6 sm:w-10 sm:h-10 bg-gradient-to-br from-violet-500 to-purple-500 rounded-md sm:rounded-lg flex items-center justify-center">
              <svg
                className="w-3 h-3 sm:w-6 sm:h-6 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                />
              </svg>
            </div>
            <div className="text-left">
              <div className="text-[10px] sm:text-xs text-gray-500">
                {t.floatingBadges.laptop.label}
              </div>
              <div className="text-xs sm:text-sm font-bold text-gray-900">
                {t.floatingBadges.laptop.title}
              </div>
            </div>
          </div>
        </div>

        <div className="absolute bottom-[22%] sm:bottom-24 left-2 sm:left-10 bg-white/80 backdrop-blur-sm px-2 py-1.5 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl shadow-md sm:shadow-lg border border-blue-200/50 pointer-events-none block">
          <div className="flex items-center gap-1 sm:gap-2">
            <div className="w-6 h-6 sm:w-10 sm:h-10 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-md sm:rounded-lg flex items-center justify-center">
              <svg
                className="w-3 h-3 sm:w-6 sm:h-6 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                />
              </svg>
            </div>
            <div className="text-left">
              <div className="text-[10px] sm:text-xs text-gray-500">
                {t.floatingBadges.smartphoneTablet.label}
              </div>
              <div className="text-xs sm:text-sm font-bold text-gray-900">
                {t.floatingBadges.smartphoneTablet.title}
              </div>
            </div>
          </div>
        </div>

        <div className="absolute bottom-[5%] sm:bottom-40 right-2 sm:right-16 bg-white/80 backdrop-blur-sm px-2 py-1.5 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl shadow-md sm:shadow-lg border border-amber-200/50 pointer-events-none block">
          <div className="flex items-center gap-1 sm:gap-2">
            <div className="w-6 h-6 sm:w-10 sm:h-10 bg-gradient-to-br from-amber-500 to-orange-500 rounded-md sm:rounded-lg flex items-center justify-center">
              <svg
                className="w-3 h-3 sm:w-6 sm:h-6 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                />
              </svg>
            </div>
            <div className="text-left">
              <div className="text-[10px] sm:text-xs text-gray-500">
                {t.floatingBadges.sameDay.label}
              </div>
              <div className="text-xs sm:text-sm font-bold text-gray-900">
                {t.floatingBadges.sameDay.title}
              </div>
            </div>
          </div>
        </div>

        <div className="absolute top-[54%] sm:top-[35%] right-6 sm:right-6 bg-white/80 backdrop-blur-sm px-2 py-1.5 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl shadow-md sm:shadow-lg border border-purple-200/50 pointer-events-none block">
          <div className="flex items-center gap-1 sm:gap-2">
            <div className="w-6 h-6 sm:w-10 sm:h-10 bg-gradient-to-br from-purple-500 to-pink-500 rounded-md sm:rounded-lg flex items-center justify-center">
              <svg
                className="w-3 h-3 sm:w-6 sm:h-6 text-white"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
                <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" />
                <path
                  clipRule="evenodd"
                  d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm9.707 5.707a1 1 0 00-1.414-1.414L9 12.586l-1.293-1.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                  fillRule="evenodd"
                />
              </svg>
            </div>
            <div className="text-left">
              <div className="text-[10px] sm:text-xs text-gray-500">
                {t.floatingBadges.original.label}
              </div>
              <div className="text-xs sm:text-sm font-bold text-gray-900">
                {t.floatingBadges.original.title}
              </div>
            </div>
          </div>
        </div>

        <div className="absolute top-20 sm:top-32 left-2 sm:left-16 bg-white/80 backdrop-blur-sm px-2 py-1.5 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl shadow-md sm:shadow-lg border border-green-200/50 pointer-events-none block">
          <div className="flex items-center gap-1 sm:gap-2">
            <div className="w-6 h-6 sm:w-10 sm:h-10 bg-gradient-to-br from-green-500 to-emerald-500 rounded-md sm:rounded-lg flex items-center justify-center">
              <svg
                className="w-3 h-3 sm:w-6 sm:h-6 text-white"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  clipRule="evenodd"
                  d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 12.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                  fillRule="evenodd"
                />
              </svg>
            </div>
            <div className="text-left">
              <div className="text-[10px] sm:text-xs text-gray-500">
                {t.floatingBadges.certified.label}
              </div>
              <div className="text-xs sm:text-sm font-bold text-gray-900">
                {t.floatingBadges.certified.title}
              </div>
            </div>
          </div>
        </div>

        <div className="absolute bottom-[24%] sm:bottom-24 right-2 sm:right-1/4 bg-white/80 backdrop-blur-sm px-2 py-1.5 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl shadow-md sm:shadow-lg border border-indigo-200/50 pointer-events-none block">
          <div className="flex items-center gap-1 sm:gap-2">
            <div className="w-6 h-6 sm:w-10 sm:h-10 bg-gradient-to-br from-indigo-500 to-blue-500 rounded-md sm:rounded-lg flex items-center justify-center">
              <svg
                className="w-3 h-3 sm:w-6 sm:h-6 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                />
              </svg>
            </div>
            <div className="text-left">
              <div className="text-[10px] sm:text-xs text-gray-500">
                {t.floatingBadges.warranty.label}
              </div>
              <div className="text-xs sm:text-sm font-bold text-gray-900">
                {t.floatingBadges.warranty.title}
              </div>
            </div>
          </div>
        </div>
        </div>

        <div className="relative z-10 mx-auto w-full max-w-[1400px] px-4 pt-4 sm:px-6 lg:pt-0">
          <div className="grid w-full max-w-full grid-cols-1 items-center gap-5 lg:grid-cols-2 lg:gap-12">
            <div className="order-1 space-y-3.5 text-center lg:space-y-5 lg:text-left">
              <div className="space-y-1">
                <h1 className="text-[1.35rem] font-bold leading-snug sm:text-3xl md:text-4xl lg:text-5xl lg:leading-tight">
                  <span className="text-gray-900">{t.hero.titleIndependent}</span>
                  <span className="bg-clip-text text-transparent bg-gradient-to-r from-violet-600 to-purple-600">
                    {t.hero.titleHighlight}
                  </span>
                  <br />
                  <span className="text-gray-900">{t.hero.titleDevices}</span>
                </h1>
              </div>
              <p className="text-xs sm:text-sm md:text-base text-gray-600 leading-relaxed px-2 sm:px-0">
                {t.hero.subtitle}
              </p>

              {/* Mobile & tablet: quote after intro */}
              <div className="scroll-mt-24 lg:hidden" id="quote-mobile">
                <ServiceQuoteBlock
                  compact
                  copy={t.serviceQuote}
                  devices={devices}
                  form={form}
                  onSubmit={submitBooking}
                />
              </div>

              {/* Desktop only — scrolls to quote section below hero */}
              <div className="hidden flex-row gap-3 lg:flex lg:justify-start">
                <a
                  className="inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-violet-600 via-purple-600 to-violet-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-violet-500/50 transition-all duration-200 hover:from-violet-700 hover:via-purple-700 hover:to-violet-700 hover:shadow-xl hover:shadow-violet-500/60"
                  href="#quote"
                >
                  {t.hero.requestQuote}
                </a>
                <Link
                  className="inline-flex items-center justify-center rounded-xl border border-violet-200 bg-white px-6 py-3 text-sm font-semibold text-violet-600 shadow-lg transition-all duration-200 hover:bg-gray-50 hover:shadow-xl group"
                  href={`tel:${process.env.NEXT_PUBLIC_BUSINESS_PHONE || "+1-555-123-4567"}`}
                >
                  <svg
                    className="mr-1.5 h-4 w-4 transition-transform duration-200 group-hover:scale-110"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 00-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z" />
                  </svg>
                  {t.hero.requestCallback}
                </Link>
              </div>

              {/* Trust indicators */}
              <div className="grid grid-cols-2 justify-items-center gap-x-3 gap-y-2 text-xs text-gray-600 sm:text-sm lg:flex lg:flex-wrap lg:justify-start lg:gap-x-4">
                <div className="flex items-center gap-1.5">
                  <svg
                    className="h-4 w-4 shrink-0 text-green-600 sm:h-5 sm:w-5"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                      clipRule="evenodd"
                      d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                      fillRule="evenodd"
                    />
                  </svg>
                  <span className="font-medium">{t.hero.yearsExperience}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <svg
                    className="h-4 w-4 shrink-0 text-green-600 sm:h-5 sm:w-5"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                      clipRule="evenodd"
                      d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                      fillRule="evenodd"
                    />
                  </svg>
                  <span className="font-medium">{t.hero.devicesServiced}</span>
                </div>
                <div className="col-span-2 flex items-center justify-center gap-1.5 lg:col-span-1">
                  <svg
                    className="h-4 w-4 shrink-0 text-green-600 sm:h-5 sm:w-5"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                      clipRule="evenodd"
                      d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                      fillRule="evenodd"
                    />
                  </svg>
                  <span className="font-medium">{t.hero.sameDayService}</span>
                </div>
              </div>
            </div>

            {/* Devices image */}
            <div className="order-2 flex justify-center lg:order-2 lg:justify-end">
              <Image
                priority
                alt={t.hero.imageAlt}
                className="w-full max-w-[350px] sm:max-w-[450px] md:max-w-[600px] lg:max-w-none lg:w-[740px] h-auto object-contain"
                height={560}
                src="/All_Devices.png"
                width={740}
              />
            </div>
          </div>
        </div>
      </section>

      {/* Request a Service Quote — desktop only (mobile is in hero) */}
      <PageSection
        className="hidden lg:block"
        id="quote"
        variant="accent"
      >
        <ServiceQuoteBlock
          copy={t.serviceQuote}
          devices={devices}
          form={form}
          onSubmit={submitBooking}
        />
      </PageSection>

      {/* About Section */}
      <PageSection id="about" variant="muted">
        <div className="relative">
          {/* Blurred background for About */}
          <div className="absolute -top-20 right-0 w-96 h-96 bg-gradient-to-br from-violet-400/20 to-purple-400/20 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-gradient-to-br from-blue-400/20 to-cyan-400/20 rounded-full blur-3xl pointer-events-none" />
          <div className="text-center mb-6 sm:mb-8 md:mb-10">
            <p className="text-violet-600 font-semibold text-xs sm:text-sm uppercase tracking-wider mb-2">
              {t.about.eyebrow}
            </p>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-3 sm:mb-4">
              {t.about.heading}
            </h2>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
            <div className="lg:col-span-2 space-y-4 sm:space-y-5">
              <p className="text-sm sm:text-base md:text-lg leading-relaxed text-gray-600">
                {t.about.body}
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div className="group p-4 sm:p-5 md:p-6 rounded-xl sm:rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 to-purple-50">
                  <div className="w-fit rounded-lg border border-violet-200 bg-white p-2 sm:p-2.5 md:p-3 mb-3 sm:mb-4">
                    <CheckCircle className="h-5 w-5 sm:h-6 sm:w-6 text-violet-600" />
                  </div>
                  <h3 className="text-base sm:text-lg md:text-xl font-bold mb-2 sm:mb-3 text-gray-900">
                    {t.about.experienceTitle}
                  </h3>
                  <p className="text-sm sm:text-base text-gray-600 leading-relaxed">
                    {t.about.experienceBody}
                  </p>
                </div>

                <div className="group p-4 sm:p-5 md:p-6 rounded-xl sm:rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-cyan-50">
                  <div className="w-fit rounded-lg border border-blue-200 bg-white p-2 sm:p-2.5 md:p-3 mb-3 sm:mb-4">
                    <Headphones className="h-5 w-5 sm:h-6 sm:w-6 text-blue-600" />
                  </div>
                  <h3 className="text-base sm:text-lg md:text-xl font-bold mb-2 sm:mb-3 text-gray-900">
                    {t.about.technicianTitle}
                  </h3>
                  <p className="text-sm sm:text-base text-gray-600 leading-relaxed">
                    {t.about.technicianBody}
                  </p>
                </div>
              </div>
            </div>

            <div className="flex items-center justify-center mt-6 lg:mt-0">
              <div className="text-center p-6 sm:p-8 md:p-10 rounded-2xl sm:rounded-3xl bg-gradient-to-br from-violet-600 to-purple-600 text-white shadow-xl shadow-violet-500/20 w-full max-w-xs">
                <div className="text-4xl sm:text-5xl md:text-6xl font-bold mb-2 sm:mb-3">
                  {t.about.statValue}
                </div>
                <p className="text-base sm:text-lg md:text-xl font-medium">
                  {t.about.statLabel}
                </p>
              </div>
            </div>
          </div>
        </div>
      </PageSection>

      {/* Services Section */}
      <PageSection id="services" variant="default">
        <div className="relative">
          {/* Blurred background for Services */}
          <div className="absolute top-0 left-10 w-72 h-72 bg-gradient-to-br from-purple-400/20 to-pink-400/20 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute top-1/2 right-10 w-64 h-64 bg-gradient-to-br from-indigo-400/20 to-violet-400/20 rounded-full blur-3xl pointer-events-none" />
          <div className="text-center mb-6 sm:mb-8 md:mb-10">
            <p className="text-violet-600 font-semibold text-xs uppercase tracking-wider mb-2">
              {t.services.eyebrow}
            </p>
            <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-3">
              {t.services.heading}
            </h2>
            <p className="text-sm sm:text-base text-default-600 max-w-2xl mx-auto px-4">
              {t.services.subtitle}
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            <div className="group p-4 sm:p-6 rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-cyan-50">
              <div className="w-fit rounded-lg border border-blue-200 bg-white p-2 sm:p-3 mb-3 sm:mb-4">
                <Droplet className="h-5 w-5 sm:h-6 sm:w-6 text-blue-600" />
              </div>
              <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3 text-gray-900">
                {t.services.waterDamage.title}
              </h3>
              <p className="text-sm sm:text-base text-gray-600">
                {t.services.waterDamage.description}
              </p>
            </div>

            <div className="group p-4 sm:p-6 rounded-2xl border border-red-200 bg-gradient-to-br from-red-50 to-orange-50">
              <div className="w-fit rounded-lg border border-red-200 bg-white p-2 sm:p-3 mb-3 sm:mb-4">
                <Cpu className="h-5 w-5 sm:h-6 sm:w-6 text-red-600" />
              </div>
              <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3 text-gray-900">
                {t.services.motherboard.title}
              </h3>
              <p className="text-sm sm:text-base text-gray-600">
                {t.services.motherboard.description}
              </p>
            </div>

            <div className="group p-4 sm:p-6 rounded-2xl border border-green-200 bg-gradient-to-br from-green-50 to-emerald-50">
              <div className="w-fit rounded-lg border border-green-200 bg-white p-2 sm:p-3 mb-3 sm:mb-4">
                <HardDrive className="h-5 w-5 sm:h-6 sm:w-6 text-green-600" />
              </div>
              <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3 text-gray-900">
                {t.services.storage.title}
              </h3>
              <p className="text-sm sm:text-base text-gray-600">
                {t.services.storage.description}
              </p>
            </div>

            <div className="group p-4 sm:p-6 rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-yellow-50">
              <div className="w-fit rounded-lg border border-amber-200 bg-white p-2 sm:p-3 mb-3 sm:mb-4">
                <Database className="h-5 w-5 sm:h-6 sm:w-6 text-amber-600" />
              </div>
              <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3 text-gray-900">
                {t.services.dataRecovery.title}
              </h3>
              <p className="text-sm sm:text-base text-gray-600">
                {t.services.dataRecovery.description}
              </p>
            </div>
          </div>
        </div>
      </PageSection>

      {/* Stats Section */}
      <PageSection id="stats" variant="muted">
        <div className="relative">
          {/* Blurred background for Stats */}
          <div className="absolute -top-10 left-1/4 w-80 h-80 bg-gradient-to-br from-violet-400/20 to-blue-400/20 rounded-full blur-3xl pointer-events-none" />
          <div className="text-center mb-6 sm:mb-8">
            <p className="text-violet-500 font-semibold text-xs uppercase tracking-wide mb-2">
              {t.stats.eyebrow}
            </p>
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-3 sm:mb-4">
              {t.stats.heading}
            </h2>
            <p className="text-xs sm:text-sm md:text-base text-default-600 max-w-4xl mx-auto">
              {t.stats.body}
            </p>
          </div>

          <div className="p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl bg-gradient-to-br from-violet-500/10 to-blue-500/10">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 md:gap-8 text-center">
              <div>
                <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold text-violet-500">
                  4,500+
                </h3>
                <p className="text-xs sm:text-sm md:text-base text-default-600 mt-1 sm:mt-2">
                  {t.stats.devicesServiced}
                </p>
              </div>
              <div>
                <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold text-violet-500">
                  500+
                </h3>
                <p className="text-xs sm:text-sm md:text-base text-default-600 mt-1 sm:mt-2">
                  {t.stats.happyCustomers}
                </p>
              </div>
              <div>
                <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold text-violet-500">
                  7+
                </h3>
                <p className="text-xs sm:text-sm md:text-base text-default-600 mt-1 sm:mt-2">
                  {t.stats.yearsExperience}
                </p>
              </div>
              <div>
                <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold text-violet-500">
                  90 Days
                </h3>
                <p className="text-xs sm:text-sm md:text-base text-default-600 mt-1 sm:mt-2">
                  {t.stats.warranty}
                </p>
              </div>
            </div>
          </div>
        </div>
      </PageSection>

      {/* Working Process */}
      <PageSection id="process" variant="default">
        <div className="relative">
          {/* Blurred background for Process */}
          <div className="absolute top-20 right-20 w-72 h-72 bg-gradient-to-br from-blue-400/20 to-cyan-400/20 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute bottom-10 left-10 w-64 h-64 bg-gradient-to-br from-purple-400/20 to-violet-400/20 rounded-full blur-3xl pointer-events-none" />
          <div className="text-center mb-6 sm:mb-8">
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3">
              {t.process.heading}
            </h2>
            <p className="text-sm sm:text-base text-default-600">
              {t.process.subtitle}
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {t.process.steps.map((step, index) => {
              const Icon = processStepIcons[index];

              return (
                <div
                  key={step.title}
                  className="relative h-full rounded-2xl border p-2 md:rounded-3xl md:p-3"
                >
                  <GlowingEffect
                    disabled={false}
                    glow={true}
                    inactiveZone={0.01}
                    proximity={64}
                    spread={40}
                  />
                  <div className="relative flex h-full flex-col justify-between gap-6 overflow-hidden rounded-xl p-6 md:p-6 dark:shadow-[0px_0px_27px_0px_#2D2D2D]">
                    <div className="relative flex flex-1 flex-col justify-between gap-5">
                      <div className="w-fit rounded-lg border border-violet-500/30 bg-violet-500/10 p-2">
                        <Icon className="h-6 w-6 text-violet-500" />
                      </div>
                      <div className="space-y-3">
                        <h3 className="font-sans text-xl font-semibold md:text-2xl">
                          {step.title}
                        </h3>
                        <p className="font-sans text-sm text-default-600 md:text-base">
                          {step.description}
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              );
            })}
          </div>
        </div>
      </PageSection>

      {/* Client Testimonials */}
      <PageSection id="testimonials" variant="muted">
        <div className="relative">
          {/* Blurred background for Testimonials */}
          <div className="absolute top-0 left-1/3 w-96 h-96 bg-gradient-to-br from-violet-400/20 to-purple-400/20 rounded-full blur-3xl pointer-events-none" />
          <div className="text-center mb-6 sm:mb-8">
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3">
              {t.testimonials.heading}
            </h2>
            <p className="text-sm sm:text-base text-default-600">
              {t.testimonials.subtitle}
            </p>
          </div>

          <InfiniteMovingCards
            className="mb-8"
            direction="right"
            items={testimonials}
            pauseOnHover={true}
            speed="slow"
          />
        </div>
      </PageSection>

      {/* FAQ Section */}
      <PageSection id="faq" variant="default">
        <div className="relative">
          <div className="absolute top-0 left-1/4 w-80 h-80 bg-gradient-to-br from-violet-400/15 to-purple-400/15 rounded-full blur-3xl pointer-events-none" />
          <div className="text-center mb-6 sm:mb-10">
            <p className="text-violet-600 font-semibold text-xs sm:text-sm uppercase tracking-wider mb-2">
              {t.faq.eyebrow}
            </p>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3">
              {t.faq.heading}
            </h2>
            <p className="text-sm sm:text-base text-gray-500 max-w-xl mx-auto">
              {t.faq.subtitle}
            </p>
          </div>

          <div className="max-w-3xl mx-auto space-y-3 relative z-10">
            {faqs.map((faq, i) => (
              <div
                key={i}
                className="border border-gray-200 rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow duration-200"
              >
                <button
                  aria-expanded={openFaq === i}
                  className="w-full flex items-center justify-between px-5 py-4 text-left"
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}
                >
                  <span className="font-semibold text-gray-900 text-sm sm:text-base pr-4">
                    {faq.q}
                  </span>
                  <ChevronDown
                    className={`h-5 w-5 text-violet-500 flex-shrink-0 transition-transform duration-200 ${openFaq === i ? "rotate-180" : ""}`}
                  />
                </button>
                {openFaq === i && (
                  <div className="px-5 pb-4 text-sm sm:text-base text-gray-600 leading-relaxed border-t border-gray-100 pt-3">
                    {faq.a}
                  </div>
                )}
              </div>
            ))}
          </div>
        </div>
      </PageSection>

      {/* Call to Action */}
      <PageSection id="contact" variant="muted">
        <div className="relative">
          {/* Blurred background for Contact */}
          <div className="absolute -top-20 right-1/4 w-80 h-80 bg-gradient-to-br from-blue-400/20 to-cyan-400/20 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute -bottom-10 left-1/4 w-72 h-72 bg-gradient-to-br from-violet-400/20 to-purple-400/20 rounded-full blur-3xl pointer-events-none" />

          <div className="relative overflow-hidden p-6 sm:p-10 md:p-14 rounded-2xl sm:rounded-3xl bg-gradient-to-br from-violet-600 via-purple-600 to-blue-600 text-white text-center shadow-2xl">
            {/* Decorative elements */}
            <div className="absolute top-0 right-0 w-48 h-48 sm:w-64 sm:h-64 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2" />
            <div className="absolute bottom-0 left-0 w-32 h-32 sm:w-48 sm:h-48 bg-white/5 rounded-full translate-y-1/2 -translate-x-1/2" />

            <div className="relative z-10">
              <div className="inline-block px-3 py-1.5 sm:px-4 sm:py-2 bg-white/20 backdrop-blur-sm rounded-full mb-3 sm:mb-4 text-xs sm:text-sm font-semibold">
                {t.contact.badge}
              </div>
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4">
                {t.contact.heading}
              </h2>
              <p className="text-sm sm:text-base md:text-lg mb-6 sm:mb-8 max-w-2xl mx-auto opacity-95">
                {t.contact.body}
              </p>
              <div className="flex gap-3 sm:gap-4 justify-center flex-wrap">
                <Link
                  className="inline-flex items-center justify-center font-bold px-6 py-3 sm:px-8 sm:py-4 rounded-full bg-white text-violet-600 shadow-xl text-sm sm:text-base"
                  href={`tel:${process.env.NEXT_PUBLIC_BUSINESS_PHONE || "+1-555-123-4567"}`}
                >
                  <svg
                    className="w-4 h-4 sm:w-5 sm:h-5 mr-2"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path
                      d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth="2"
                    />
                  </svg>
                  <span className="hidden sm:inline">
                    {t.contact.requestCallbackDesktop}{" "}
                    {process.env.NEXT_PUBLIC_BUSINESS_PHONE ||
                      "+1-555-123-4567"}
                  </span>
                  <span className="sm:hidden">{t.contact.requestCallbackMobile}</span>
                </Link>
                <Link
                  className="inline-flex items-center justify-center font-bold px-6 py-3 sm:px-8 sm:py-4 rounded-full border-2 border-white text-white text-sm sm:text-base"
                  href={`https://wa.me/${(process.env.NEXT_PUBLIC_BUSINESS_PHONE || "+15551234567").replace(/[^0-9]/g, "")}`}
                >
                  <svg
                    className="w-4 h-4 sm:w-5 sm:h-5 mr-2"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                  </svg>
                  {t.contact.whatsapp}
                </Link>
              </div>
            </div>
          </div>
        </div>
      </PageSection>

      {/* Site Footer */}
      <footer className="w-full bg-gray-50 border-t border-gray-200 mt-0">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-8 sm:py-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            <div>
              <h3 className="font-bold text-gray-900 mb-2 text-sm uppercase tracking-wider">
                {dict.nav.brand}
              </h3>
              <p className="text-sm text-gray-500 leading-relaxed">
                {t.footer.tagline}
                <br />
                {t.footer.taglineLine2}
              </p>
            </div>
            <div>
              <h3 className="font-bold text-gray-900 mb-2 text-sm uppercase tracking-wider">
                {t.footer.contactHours}
              </h3>
              <ul className="space-y-1.5 text-sm text-gray-500">
                <li>ðŸ“ {t.footer.address}</li>
                <li>
                  ðŸ“ž{" "}
                  <a
                    className="hover:text-violet-600 transition-colors"
                    href={`tel:${process.env.NEXT_PUBLIC_BUSINESS_PHONE || "+1-934-242-2350"}`}
                  >
                    {process.env.NEXT_PUBLIC_BUSINESS_PHONE || "+1-934-242-2350"}
                  </a>
                </li>
                <li>
                  âœ‰ï¸{" "}
                  <a
                    className="hover:text-violet-600 transition-colors"
                    href={`mailto:${process.env.NEXT_PUBLIC_BUSINESS_EMAIL || "support@deviceservicenyc.com"}`}
                  >
                    {process.env.NEXT_PUBLIC_BUSINESS_EMAIL || "support@deviceservicenyc.com"}
                  </a>
                </li>
                <li className="pt-1">
                  <span className="font-medium text-gray-600">{t.footer.monFri}</span>{" "}
                  {t.footer.monFriHours}
                </li>
                <li>
                  <span className="font-medium text-gray-600">{t.footer.saturday}</span>{" "}
                  {t.footer.saturdayHours}
                </li>
                <li>
                  <span className="font-medium text-gray-600">{t.footer.sunday}</span>{" "}
                  {t.footer.sundayHours}
                </li>
              </ul>
            </div>
            <div>
              <h3 className="font-bold text-gray-900 mb-2 text-sm uppercase tracking-wider">
                {t.footer.quickLinks}
              </h3>
              <ul className="space-y-1.5 text-sm text-gray-500">
                <li>
                  <Link
                    className="hover:text-violet-600 transition-colors"
                    href="/#services"
                  >
                    {t.footer.repairServices}
                  </Link>
                </li>
                <li>
                  <Link
                    className="hover:text-violet-600 transition-colors"
                    href="/pricing"
                  >
                    {t.footer.pricing}
                  </Link>
                </li>
                <li>
                  <Link
                    className="hover:text-violet-600 transition-colors"
                    href="/#faq"
                  >
                    {t.footer.faqs}
                  </Link>
                </li>
                <li>
                  <Link
                    className="hover:text-violet-600 transition-colors"
                    href="/#testimonials"
                  >
                    {t.footer.reviews}
                  </Link>
                </li>
              </ul>
            </div>
            <div>
              <h3 className="font-bold text-gray-900 mb-2 text-sm uppercase tracking-wider">
                {t.footer.legal}
              </h3>
              <ul className="space-y-1.5 text-sm text-gray-500">
                <li>
                  <Link
                    className="hover:text-violet-600 transition-colors"
                    href="/privacy"
                  >
                    {t.footer.privacyPolicy}
                  </Link>
                </li>
                <li>
                  <Link
                    className="hover:text-violet-600 transition-colors"
                    href="/terms"
                  >
                    {t.footer.termsConditions}
                  </Link>
                </li>
                <li>
                  <Link
                    className="hover:text-violet-600 transition-colors"
                    href="/#contact"
                  >
                    {t.footer.contactUs}
                  </Link>
                </li>
              </ul>
            </div>
          </div>

          {/* Google Map Embed */}
          <div className="mb-6 rounded-xl overflow-hidden border border-gray-200" style={{ height: "220px" }}>
            <iframe
              allowFullScreen
              height="220"
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
              src="https://maps.google.com/maps?q=14+Terry+St,+Hicksville,+NY+11801&z=15&output=embed"
              style={{ border: 0, width: "100%", height: "100%" }}
              title={t.footer.mapTitle}
            />
          </div>

          <div className="border-t border-gray-200 pt-5 flex flex-col gap-2 text-xs text-gray-400">
            <p className="text-center">
              {t.footer.disclaimer}
            </p>
            <p className="text-center">
              &copy; {new Date().getFullYear()} {dict.nav.brand}. {t.footer.copyright}
            </p>
          </div>
        </div>
      </footer>

      {/* Sticky Mobile CTA Bar */}
      <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-white border-t border-gray-200 shadow-2xl px-4 py-3 flex gap-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
          <button
            className="flex-1 inline-flex items-center justify-center font-bold px-4 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 text-white text-sm shadow-md min-h-11"
            type="button"
            onClick={() => {
              document
                .querySelector("#quote-mobile, #quote")
                ?.scrollIntoView({ behavior: "smooth", block: "start" });
            }}
          >
            {t.stickyCta.requestQuote}
          </button>
          <a
            className="flex-1 inline-flex items-center justify-center font-bold px-4 py-3 rounded-xl border-2 border-violet-600 text-violet-600 text-sm min-h-11"
            href={`tel:${process.env.NEXT_PUBLIC_BUSINESS_PHONE || "+1-555-123-4567"}`}
          >
            <svg
              className="w-4 h-4 mr-1.5"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
              <path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 00-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z" />
            </svg>
            {t.stickyCta.requestCallback}
          </a>
      </div>
      <div className="h-20 lg:hidden" />
    </div>
  );
}
